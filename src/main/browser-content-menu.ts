import { app, BaseWindow, clipboard, Menu, MenuItem, WebContentsView } from 'electron';
...
const getBaseWindowFromWebContents = (
  ...
  return BaseWindow.getAllWindows().find((win) => {
    ...
    return children.some((view) => view.webContents === webContents);
  });
};
...
export const buildChromeContextMenu = (opts: ChromeContextMenuOptions): Menu => {
  ...
  const menu = new Menu();
  const append = (opts: Electron.MenuItemConstructorOptions): void =>
  ...
  if (menu.items.length === 0) {
    const browserWindow = getBaseWindowFromWebContents(webContents);
